...
<template>
  <v-responsive class="border rounded">
    <v-app
      :theme="theme"
      style="background: linear-gradient(to left, #e1ebdc, #a9c6a9); font-family: 'Roboto', sans-serif " 
    >
      <!-- App Bar -->
      <v-app-bar :style="{ background: 'linear-gradient(to right, #c4f0c4, #3b803b)' }" class="px-3 nav-bar"> <!-- Updated gradient -->
        <v-app-bar-nav-icon
          @click="drawer = !drawer"
          style="color: white"
          class="d-lg-none"
      ...
      <v-main>
        <v-container fluid>
          <v-img src="public/pexels-ivan-drazic-20457695-20385805.jpg" height="400px" cover>
            <div class="b-text">College of Forestry and Environmental Science</div>
          </v-img>
          ...
          <v-container class="text-center mt-8">
            <h1 class="text-h4 font-weight-bold mb-4" style="font-size: 2rem;">
              "Represent your love for forests and the earth with our eco-inspired merch."
            </h1>
            <p class="text-body-1" style="font-size: 1rem; font-weight: bold; max-width: 900px; margin: 0 auto;">
              "Celebrate your passion for the natural world with gear made for you."
            </p>
          </v-container>
    ...
    <v-dialog v-model="modalVisible" max-width="800px">
      <v-card class="b-card">
        ...
        <v-card-actions class="d-flex justify-end">
          <v-btn class="b-btn" @click="buyNow">Buy Now</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
  ...
  <v-footer class="b-footer" padless>
...
.b-card {
  background: linear-gradient(135deg, #0c9f27, #24e68e);
  color: white;
  border-radius: 10px;
}
...
.v-btn.b-btn {
  background: linear-gradient(45deg, #2fee8b, #11c626);
  color: white;
  font-weight: bold;
  border-radius: 5px;
...
.v-btn.b-btn:hover {
  background: linear-gradient(45deg, #11c626, #2fee8b);
}
